...
int BitcoinExchange::read_and_parsFILE_SCV(std::string path)
{
    ...
    while (std::getline(file, line))
    {
        ...
        if (check_formatofDate(year, month, day, prix))
            ...
            throw std::string("Error: not a positive prix.");
        else if (std::stod(prix) > MAX_INT)
            throw std::string("Error: too large a prix");
    ...
    while (std::getline(file, line))
    {
        ...
        else
        {
            ...
            if (this->find(bitc) == -1)
                ...
                std::cout << bitc.getFormat() << " => " << value << " = " << this->find(bitc) * bitc.getValue() << std::endl;
        }
        i++;
    }
    return 0;
...
float BitcoinExchange::find(Bitcoin &bitc)
{
...
BitcoinExchange::BitcoinExchange(std::string path)
{
    ...
    // std::map<std::string, Bitcoin>::iterator it = _mapscv.begin();
    // int i = 0;
}
BitcoinExchange::~BitcoinExchange()
{
